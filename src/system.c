...
// BIG ENDIAN FORMS
uint16_t read_instr_be(system_bus_t *sys, uint16_t address, bool is_word) {
...
// LITTLE ENDIAN FORMS 
uint16_t read_mem(system_bus_t *sys, uint16_t address, bool is_word) {
  uint16_t byte_width = is_word ? 2 : 1;
  uint16_t low = sys->memory[address];
...
void write_mem(system_bus_t *sys, uint16_t address, uint16_t value, bool is_word) {
  uint16_t byte_width = is_word ? 2 : 1;
  uint8_t low = value & 0xFF;
  uint8_t high = (value >> 8) & 0xFF;
